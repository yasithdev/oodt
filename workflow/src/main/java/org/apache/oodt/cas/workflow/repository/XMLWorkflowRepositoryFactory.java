...
public class XMLWorkflowRepositoryFactory implements WorkflowRepositoryFactory {
	...
	/* our log stream */
	private static Logger LOG = LoggerFactory.getLogger(XMLWorkflowRepositoryFactory.class.getName());
			...
			if (recursive) {
            	...
            	for (String rootDir : dirUris) {
            		try {
            			...
            			LOG.warn("URISyntaxException when traversing directory: "+rootDir);
            		}
            	}        
	...
	public WorkflowRepository createRepository() {
		if(workflowDirList != null){
			...
			LOG.warn("Cannot create XML Workflow Repository: no workflow dir uris specified: value: "+System.getProperty("org.apache.oodt.cas.workflow.repo.dirs"));
			return null;
		}
	}

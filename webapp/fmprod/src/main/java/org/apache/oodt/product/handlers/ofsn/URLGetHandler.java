...
public class URLGetHandler extends AbstractCrawlLister implements OFSNGetHandler {
	...
	Logger LOG = LoggerFactory.getLogger(URLGetHandler.class.getName());
	...
	public byte[] retrieveChunk(String filepath, long offset, int length)
		...
		try {
			...
			LOG.warn("Unable to obtain byte chunk ("+offset+" - "+(offset+length)+") " 
	...
	public long sizeOf(String filepath) {
		...
		try {
			...
			LOG.warn("Unable to obtain size information for filepath listing ["+filepath+"]");
			LOG.warn(e.getMessage());
		} catch (IllegalArgumentException e) {
			LOG.warn("Unable to obtain size information for filepath listing ["+filepath+"]");
			LOG.warn(e.getMessage());

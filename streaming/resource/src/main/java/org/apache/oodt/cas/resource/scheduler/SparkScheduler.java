...
public class SparkScheduler implements Scheduler {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(SparkScheduler.class.getName());
        ...
        while (true) {
            try {
                ...
                if (!(clazz.newInstance() instanceof SparkInstance)) {
                    LOG.warn("Non-Spark job found ("+spec.getJob().getId()+") ignoring.");
                    continue;
                }
                this.schedule(spec);
            } catch(SchedulerException e) {
                LOG.warn("Scheduler exception detected: "+e.getMessage());
//            } catch (JobQueueException e) {
//                LOG.warn("Could not get next job from job-queue.");
            } catch (ClassNotFoundException e) {
                LOG.warn("Class not found: "+e.getMessage());
            } catch (InstantiationException e) {
                LOG.warn("Could not instantiate: "+e.getMessage());
            } catch (IllegalAccessException e) {
                LOG.warn("Could not access: "+e.getMessage());
            }
    ...
    public boolean schedule(JobSpec spec) throws SchedulerException {
        try {
            ...
            LOG.warn("Job input exception detected.");
